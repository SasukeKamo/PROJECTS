...
class EventListener:
    ...
    def display_footer(self):
        ...
        if self.page == 0 and len(self.events) > 30:
            footer_text = "Nacisnij (n) aby przejsc do nastepnej strony"
        ...
        if self.page == int(len(self.events) / 30) and self.page > 0:
            footer_text = "Nacisnij (p) aby przejsc do poprzedniej strony"
            text_surface = self.FOOTER_FONT.render(footer_text, True, (0, 0, 0))
            self.surface.blit(text_surface, (540, 500))

        if self.page > 0 and self.page < int(len(self.events) / 30):
            footer_text_1 = "Nacisnij (n) aby przejsc do nastepnej strony"
            footer_text_2 = "Nacisnij (p) aby przejsc do poprzedniej strony"
